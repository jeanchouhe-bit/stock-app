...
st.title("📈 股票分拣终端 v15.0 (错位双扫版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | 🧠 **终极算法:** 错位双重扫描，物理级防漏网！")
...
def local_unlimited_ocr(uploaded_file, show_debug=False):
    ...
    w, h = img.size
    max_h = 1500  # 恢复到创纪录的 1500 像素黄金尺寸
    all_text = ""
    
    is_long_image = h > max_h
    
    if is_long_image:
        st.info("📏 启动【长图错位双重扫描】，正在拉起两张交叉网，确保零漏网之鱼...")
        # 💡 核心大招：生成两套切片网格
        # Pass 1: 0, 1500, 3000...
        # Pass 2: 750, 2250, 3750... (完美覆盖 Pass 1 的切割缝隙)
        cut_points = list(range(0, h, max_h)) + list(range(750, h, max_h))
        cut_points.sort() # 排序只是为了进度展示顺畅
    else:
        st.info("📏 启动【短图纳米锐化模式】，专注解除像素粘连...")
        cut_points = [0] # 短图不切，或者只切一次
...
# 交互界面：呼叫本地算力
# ==========================================
with st.expander("📸 展开使用【纯本地智能扫图】 (自动防漏网切片)", expanded=True):
    ...
    if uploaded_file is not None:
        ...
        with col2:
            with st.spinner("🧠 错位双重扫描网已撒下，正在打捞全部代码..."):
                try:
                    ...
                    if unique_codes:
                        unique_codes.sort()
                        st.success(f"🎉 破解完毕！双网打捞成功提取 {len(unique_codes)} 只股票！")
                        auto_codes = ", ".join(unique_codes)
                        st.code(auto_codes)
                    else:
                        st.error("未能提取到代码。请查看下方 X光透视。")
                        
                    with st.expander("🛠️ 查看 AI 眼里的原始字符 (双网合并去重前)"):
