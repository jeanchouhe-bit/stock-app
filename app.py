...
st.title("📈 股票分拣终端 v14.3 (物理降噪版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | ♾️ **完全自由:** 纯本地 Tesseract + 黑客级降维降噪显微算法，0接口限制！")

# ==========================================
# (!! 核心黑科技 !!) 本地无限制切片与显微增强引擎
# ==========================================
def local_unlimited_ocr(uploaded_file, show_debug=False):
    """纯 Python 图像处理 + 本地 Tesseract (物理降噪版)"""
    ...
    if slices > 1:
        st.info(f"📏 长图已切割为 {slices} 个高清碎片，正在进行降噪扫描...")
    
    for i in range(0, h, max_h):
        box = (0, i, w, min(i + max_h, h))
        chunk = img.crop(box)
        
        # --- 显微镜级图像手术 (V14.3 物理降噪版) ---
        # 1. (!!核心) 降维打击：去除噪音沙粒
        # 使用 MedianFilter，它能抹平极小的像素噪音，但保留笔画边缘
        ...
        # 短图容易被顶部白色区域骗，这里提高检测范围，并稍微降低阈值
        if np.mean(img_np) < 140: 
            chunk_gray = ImageOps.invert(chunk_gray)
            
        # 5. (!!核心) 强力对比拉伸
        # 使用激进的 cutoff，将最暗的 3% 像素变黑，最亮的 3% 变白
        # 噪音去掉后，我们可以放心用高对比度
        chunk_final = ImageOps.autocontrast(chunk_gray, cutoff=3)
        
        # 6. X光透视仪
        if show_debug:
            st.image(chunk_final, caption=f"🔍 X光透视：切片 {i//max_h + 1} (经过物理降噪与强力拉伸)", use_container_width=True)
        
        # 7. 召唤 Tesseract (放弃双核，回归最准的 PSM 6 表格模式)
        try:
            # 噪音除尽后，规整模式最准
...
# 交互界面：呼叫本地算力
# ==========================================
with st.expander("📸 展开使用【纯本地无损扫图】 (已开启物理降噪)", expanded=True):
    # 新增：视觉调试开关
    ...
    if uploaded_file is not None:
        ...
        with col2:
            with st.spinner("🤖 降噪算法开启，正在净化图片中的数字信息..."):
                try:
                    ...
                    if unique_codes:
                        unique_codes.sort()
                        st.success(f"🎉 物理降噪完毕！成功榨取 {len(unique_codes)} 只股票！")
