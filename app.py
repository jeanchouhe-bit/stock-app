...
st.title("📈 股票分拣终端 v14.4 (显微锐化版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | ♾️ **完全自由:** 纯本地极致降噪 + 暴力对比度放大！")

# ==========================================
# (!! 核心黑科技 !!) 本地无限制切片与显微增强引擎
# ==========================================
def local_unlimited_ocr(uploaded_file, show_debug=False):
    """纯 Python 图像处理 + 本地 Tesseract (暴力对比度版)"""
    ...
    if slices > 1:
        st.info(f"📏 长图已切割为 {slices} 个高清碎片，正在进行降噪与高对比锐化...")
    
    for i in range(0, h, max_h):
        box = (0, i, w, min(i + max_h, h))
        chunk = img.crop(box)
        
        # --- 显微镜级图像手术 (V14.4 暴力锐化版) ---
        # 1. 降维打击：去除噪音沙粒
        ...
        # 5. 自动拉伸 (切掉上下 5% 的杂色像素，比上一版更激进)
        chunk_final = ImageOps.autocontrast(chunk_gray, cutoff=5)
        
        # 6. (!!新增杀手锏!!) 物理对比度放大器
        # 将现有的对比度强行乘以 2.5 倍，把浅灰色彻底逼成纯黑或纯白
        enhancer = ImageEnhance.Contrast(chunk_final)
        chunk_final = enhancer.enhance(2.5)
        
        # 7. X光透视仪
        if show_debug:
            st.image(chunk_final, caption=f"🔍 X光透视：切片 {i//max_h + 1} (经过 2.5 倍物理对比度放大)", use_container_width=True)
        
        # 8. 召唤 Tesseract (PSM 6 表格模式)
        try:
...
# 交互界面：呼叫本地算力
# ==========================================
with st.expander("📸 展开使用【纯本地无损扫图】 (已开启暴力对比度增强)", expanded=True):
    # 新增：视觉调试开关
    ...
    if uploaded_file is not None:
        ...
        with col2:
            with st.spinner("🤖 降噪与强力锐化算法开启，正在净化图片..."):
                try:
