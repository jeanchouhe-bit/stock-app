...
st.title("📈 股票分拣终端 v14.5 (纳米锐化版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | ♾️ **完全自由:** 纯本地极致锐化，专治像素粘连与模糊！")

# ==========================================
# (!! 核心黑科技 !!) 本地无限制切片与显微增强引擎
# ==========================================
def local_unlimited_ocr(uploaded_file, show_debug=False):
    """纯 Python 图像处理 + 本地 Tesseract (纳米锐化版)"""
    ...
    if slices > 1:
        st.info(f"📏 长图已切割为 {slices} 个高清碎片，正在启动纳米级锐化扫描...")
    
    for i in range(0, h, max_h):
        box = (0, i, w, min(i + max_h, h))
        chunk = img.crop(box)
        
        # --- 显微镜级图像手术 (V14.5 纳米锐化版) ---
        # 1. 撤销破坏性的模糊滤镜，直接无损放大 2.5 倍
        cw, ch = chunk.size
...
# 交互界面：呼叫本地算力
# ==========================================
with st.expander("📸 展开使用【纯本地无损扫图】 (已开启纳米锐化防粘连)", expanded=True):
    debug_mode = st.checkbox("🔍 开启【X光透视】 (排查图片识别失败原因必点)", value=True) # 默认开启帮你测试
    ...
    if uploaded_file is not None:
        ...
        with col2:
            with st.spinner("🤖 边缘锐化算法开启，正在切割字体的粘连部分..."):
                try:
                    ...
                    if unique_codes:
                        ...
                        st.error("未能提取到代码。请务必查看下方 X光透视，确认字迹是否清晰独立。")
