...
st.title("📈 股票分拣终端 v14.1 (显微调优版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | ♾️ **完全自由:** 纯本地 Tesseract + 温和切片显微算法，0接口限制！")

# ==========================================
# (!! 核心黑科技 !!) 本地无限制切片与显微增强引擎
# ==========================================
def local_unlimited_ocr(uploaded_file):
    """纯 Python 图像处理 + 本地 Tesseract 识别 (温和解绑版)"""
    ...
    for i in range(0, h, max_h):
        ...
        # 3. 智能判断暗黑模式并反色 (因为 Tesseract 只认识白底黑字)
        img_np = np.array(chunk_gray)
        if np.mean(img_np) < 127: # 如果图片整体偏暗
            chunk_gray = ImageOps.invert(chunk_gray)
            
        # 4. 温和拉伸对比度 (删除了之前毁灭性的暴力二值化)
        chunk_final = ImageOps.autocontrast(chunk_gray)
        
        # 5. 召唤本地 Tesseract (PSM 11 稀疏模式)
        try:
            # 开启测试模式：你可以取消下面这行的注释，看看图片被我们处理成什么样了
            # st.image(chunk_final, caption=f"切片 {i//max_h + 1} 喂给 AI 的真实画面", use_container_width=True)
...
with st.expander("📸 展开使用【纯本地无损扫图】 (无视接口限制，随意扫)", expanded=True):
    ...
    if uploaded_file is not None:
        ...
        with col2:
            with st.spinner("🤖 正在进行温和显微手术，并呼叫本地 Tesseract 引擎..."):
                try:
                    ...
                    if unique_codes:
                        ...
                        st.error("引擎未能提取到代码。请展开下方查看 AI 眼里的字符，判断是否需要调参。")
                        
                    with st.expander("🛠️ 查看 AI 眼里的原始字符"):
                        st.text(text if text.strip() else "【完全空白，说明图片被处理得太白或太黑，AI什么都没看到】")
                except Exception as e:
                    st.error(f"发生意外错误: {e}")
st.markdown("---")
