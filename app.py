...
st.title("📈 股票分拣终端 v14.7 (重叠防漏版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | 🧠 **智能大脑:** 动态配方 + 重叠切片，一网打尽所有代码！")
...
def local_unlimited_ocr(uploaded_file, show_debug=False):
    ...
    # 引入重叠切片法 (Sliding Window) 解决“断头台”误伤问题
    max_h = 1500  # 每张切片高度
    step = 1350   # 步长，确保有 150px 的重叠区
    all_text = ""
    ...
    if is_long_image:
        st.info(f"📏 检测到超长截图，启动【长图暴力降噪模式】，并采用重叠切法分为 {slices} 段...")
    else:
        st.info("📏 检测到短图，启动【纳米锐化模式】，专注解除像素粘连...")
    ...
    for i in range(0, h, step):
        ...
        if show_debug:
            mode_name = "暴力重叠" if is_long_image else "纳米锐化"
            st.image(chunk_final, caption=f"🔍 X光透视：重叠切片段落 (使用【{mode_name}】算法)", use_container_width=True)
