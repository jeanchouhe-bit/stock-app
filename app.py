...
st.title("📈 股票分拣终端 v15.1 (满血大满贯版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | 🧠 **终极算法:** 错位双重扫描 + 断尾防干扰提取 + 全面支持ETF！")
...
def local_unlimited_ocr(uploaded_file, show_debug=False):
    ...
    if is_long_image:
        st.info("📏 启动【长图错位双重扫描】，交叉网已拉起，确保零漏网之鱼...")
        # 错位网格：0, 750, 1500, 2250...
        cut_points = list(range(0, h, max_h)) + list(range(750, h, max_h))
        cut_points.sort() 
    else:
        st.info("📏 启动【短图纳米锐化模式】，专注解除像素粘连...")
        cut_points = [0] 
    ...
    for i in cut_points:
        ...
        if show_debug:
            st.image(chunk_final, caption=f"🔍 X光透视：Y轴起点 {i}px", use_container_width=True)
        
        try:
            text_6 = pytesseract.image_to_string(chunk_final, config='--psm 6')
            # 💡 强力容错补丁：处理常见的字母变异 (O->0, I->1, S->5, U->0, B->8)
...
# ==========================================
# 允许放行的股票/ETF前缀
VALID_PREFIXES = ('60', '68', '00', '30', '15', '51', '56')

with st.expander("📸 展开使用【纯本地智能扫图】 (已解决连体字与ETF漏抓)", expanded=True):
    ...
    if uploaded_file is not None:
        ...
        with col2:
            with st.spinner("🧠 断尾防干扰提取已开启，正在过滤杂质..."):
                try:
                    text = local_unlimited_ocr(uploaded_file, show_debug=debug_mode)
                    
                    # 💡 终极断尾提取法：先找出所有长度 >= 6 的数字串，然后只取最后 6 位，彻底无视前面的图标寄生虫
                    raw_chunks = re.findall(r'\d{6,}', text)
                    codes = [chunk[-6:] for chunk in raw_chunks if chunk[-6:].startswith(VALID_PREFIXES)]
                    unique_codes = list(set(codes))
                    
                    if unique_codes:
                        unique_codes.sort()
                        st.success(f"🎉 破解完毕！大满贯提取 {len(unique_codes)} 只标的 (包含ETF)！")
                        auto_codes = ", ".join(unique_codes)
                        st.code(auto_codes)
                    else:
                        st.error("未能提取到有效代码。")
                        
                    with st.expander("🛠️ 查看 AI 眼里的原始字符 (找茬专用)"):
...
if st.button("🚀 启动极速分拣", use_container_width=True):
    ...
    for raw in raw_list:
        chunks = re.findall(r'\d{6,}', raw)
        for chunk in chunks:
            code = chunk[-6:]
            if code.startswith(VALID_PREFIXES):
    ...
    if not clean_stocks:
        ...
        with st.spinner(f"⚡ 正在为 {len(clean_stocks)} 只标的执行动能与空间计算..."):
