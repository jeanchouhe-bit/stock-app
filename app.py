...
st.title("📈 股票分拣终端 v14.9 (黄金大块版)")
st.markdown("⚡ **双核引擎:** 腾讯实盘 + BS历史 | 🧠 **智能大脑:** 极限大块切割，把切片误伤率降到最低！")
...
def local_unlimited_ocr(uploaded_file, show_debug=False):
    ...
    # 💡 终极校准：既然切刀会伤人，那就尽量少切！
    # 扩大到 2500 像素切一刀（原本是 1500 和 800），直接减少 60% 的切割次数！
    max_h = 2500  
    all_text = ""
    ...
    if is_long_image:
        st.info(f"📏 启动【黄金大块模式】，长图仅分为 {slices} 个超大片段，最大限度保留排版完整性...")
    else:
        st.info("📏 启动【纳米锐化模式】，专注解除短图的像素粘连...")
    
    for i in range(0, h, max_h):
        # 干净利落，不搞重叠，防止半截字干扰 AI 的基准线计算
        ...
        if show_debug:
            mode_name = "暴力降噪(超大块)" if is_long_image else "纳米锐化"
            st.image(chunk_final, caption=f"🔍 显微透视：第 {i//max_h + 1}/{slices} 块 (算法: {mode_name})", use_container_width=True)
...
# 交互界面：呼叫本地算力
# ==========================================
with st.expander("📸 展开使用【纯本地智能扫图】 (自动防误伤切片)", expanded=True):
    ...
    if uploaded_file is not None:
        ...
        with col2:
            with st.spinner("🧠 黄金超大块切割中，最大限度保护排版..."):
                try:
                    ...
                    if unique_codes:
                        unique_codes.sort()
                        st.success(f"🎉 破解完毕！极限扫描成功提取 {len(unique_codes)} 只股票！")
